...
function checkPerUnitExceptionStatusForBEAtLineLevelForOCRStatic(){
    alert("in checkPerUnitExceptionStatusForBEAtLineLevelForOCRStatic")
            ...
            try{
                 mydb.transaction(function (t) {
                    t.executeSql("INSERT INTO businessExpDetails (accHeadId,expNameId,expDate,expFromLoc,expToLoc,expNarration,expUnit,expAmt,currencyId,isEntitlementExceeded,busExpAttachment,wayPointunitValue) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", 
                        [accHeadId,expNameId,expDate,fromLoc,toLoc,narration,unit,amount,currencyId,isEntitlementExceeded,file,'null']);
            ...
            if (result != null && result.rows != null) {
                ...
                for ( record = 0; record< result.rows.length; record++) {   
                    ...
                    var data ="<li class='swipeout'> <div class='swipeout-content'> "
                        ...
                        + " <ul> "
                        + " <li> "
                        + " <a onclick='capturePhotoDynamic('BE',"+i+");'' href='' class='vericonssml'><i class='fa fa-camera' aria-hidden='true'></i></a> "
                        + " </li> "
                        + " <li> "
                        + " <a href='' class='vericonssml'><i class='fa fa-picture-o' aria-hidden='true'></i></a> "
                        + " </li> "
...
setTimeout(function(){ 
   if(type == "S"){
        ...
        currency_id=j("#currency").select2('data').id;
        if(fileTempGalleryBE ==undefined || fileTempGalleryBE ==""){
            //alert("in undefined gallery send for approval")
        
        }else{
            file = fileTempGalleryBE;
            //alert("in undefined camera send for approval")
        }
...
function capturePhoto(voucher_type) {
    alert("in capture")
...
function onFail(){
    ...
    function getPhoto(source,voucher_type) {
        alert("in gallery")
        voucherType = voucher_type; 
      navigator.camera.getPicture(onPhotoURISuccess, onFail, { quality: 10, 
        destinationType: 0,
...
function viewMapBE(){
    alert("in viewMapBE")
        ...
        fromLoc = document.getElementById("ocrFromLoc");
        toLoc = document.getElementById("ocrToLoc");
        unitValue = document.getElementById("ocrUnit");
        wavepoint = document.getElementById("wayPointunitValue");
        if(fromLoc.value!='' && toLoc.value!=''){
            calculateAndDisplayRouteBE();
            document.getElementById("mapImage").setAttribute('disabled', false);
        }
    }


    function calculateAndDisplayRouteBE() {
        alert(" in calculateAndDisplayRouteBE")
        ...
        // Create a renderer for directions and bind it to the map.
          ...
          directionsDisplay.addListener('directions_changed', function() {
            computeTotalDistanceBE(directionsDisplay.getDirections());
            });
          var points=[];
    ...
    function computeTotalDistanceBE(result) {
        alert("in computeTotalDistanceBE")
        unitValue = document.getElementById("ocrUnit");
    ...
    function returnUnitResultBE(){
        alert("in returnUnitResultBE")
        ...
        if (flagForUnitEnable == true){
            unt = document.getElementById("ocrUnit");
            amt = document.getElementById("ocrAmount");
            document.getElementById("ocrAmount").value = parseFloat(Math.round(parseFloat(unt.value)) * parseFloat(ratePerUnit.value));
            checkPerUnitExceptionStatusForBEAtLineLevelForOCRStatic();
        }
    }   
/*
    function capturePhotoDynamic(voucher_type,i){
        voucherType = voucher_type; 
        navigator.camera.getPicture(onPhotoDataSuccessDynamic(i), onFailDynamic, { quality: 10,
            destinationType: 0 });

}

function onPhotoDataSuccessDynamic(i){
   resetImageData();
        if(voucherType == 'BE'){
        smallImageBE.style.display = 'block'; 
        fileTempCameraBE_+i = "data:image/jpeg;base64," + imageData;
        smallImageBE.src = "data:image/jpeg;base64," + imageData;
        fileTempGalleryBE ="";
    }
}*/